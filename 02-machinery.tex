...
((More))
By the lorentzian splitting theorem $(M,g)$ is isometric to the product $(\R\times V,-dt^2 \oplus h)$ where $(V,h)$ is a complete riemannian manifold. We will denote $T:M\to \R$ the projection onto the first component.
...
\begin{lemma}\label{lem:Kcharact}
    ...
    \begin{enumerate}[label={\textnormal{(\arabic*)}}]
        ...
        $\Theta:S^{n-1}\times[0,1)\to K\setminus p^+$ is a diffeomorphism, and 
        \[
            T(t):= T(\Theta(a,t)) \text{ in independent of }a\in \sn.
        \]
