...
%     Now again by the chain rule we have $0=\x_v(b,0) = d\exp_q\rvert_{bw} \circ \frac{d}{dv}(bx_u(0,v))\rvert_{v=0}$. But since $\xi \coloneqq  \frac{d}{dv}(bx_u(0,v))\rvert_{v=0} \in T_{bw}(L_qM)$ we are done.
% \end{proof}
...
% We now set $\Theta\coloneqq \exp_{p^+}\circ \; \theta$, which satisfies all properties in (2) and are done with this part.
...
\begin{figure}\label{fig:Transversality}
    \centering
    \input{graphics/Transversality.tex}
...
The core idea of the next lemma is that $K$ is a subset of $J^-(p^+)$. $J^-(p^+)$ has, by definition, the useful property that once a lightlike curve leaves it it can never return.
...
\begin{definition}[Observer]
    ...
    \end{align*}
    where $v_a$ is given by \refeq{eq:CLMEQSN} and $T_a$ will be defined in the next lemma.
\end{definition}
...
% Owing to equation \eqref{eq:constspeed} also have 
...
\begin{figure}\label{fig:DirReconstr}
    \centering
    \input{graphics/DirReconstr.tex}
...
\begin{definition}[Observation Preimage]\label{def:preimage}
...
We now aim to show that for $q\in V$, $\mc{P}_K(q)$ is locally the finite union of submanifolds. This will make use of a lot of the machinery developed so far. 
...
\begin{figure}\label{fig:UnionOfManif}
    \centering
    \input{graphics/UnionOfManif.tex}
    \caption{
...
Note that we can use $f_a(q)$ to define the related functions \begin{align*}
...
After defining the observation time functions and the earliest observation point for a $q\in V$ and observer $\mu_a, a\in \sn$ we can now look at the set of all earliest observation points together:
...
Finally the proof of (5) is analogous to (2) with the difference in submanifold dimension originating from the face that for any $(p,v)\in \mc{D}^{reg}_K(q)$ we also have $(p,cv)\in \mc{D}^{reg}_K(q)$ for all $c\in \R_+$.
\end{proof}
...
\begin{proof}
   ...
   Now by definition for any $p\in \ee(q)$, we have 
    \[
        \mc{D}_p \coloneqq  \pi^{-1}(p)\cap \mc{D}_K(q) = \{(p,\R_+v_1), \dots, (p,\R_+v_N)\}
    \]
...
In the following we will for any $(q,p)\in \mc{Z}$ denote $v(q,p)\coloneqq \gamma'_{q,w(q,p)}(1)$, i.e. the velocity vector of the unique geodesic from $q$ to $p$ at $p$. Sometimes we can only recover the direction of $v(q,p)$ and will denote $\cl{v}(q,p) = \frac{v(q,p)}{\lVert v(q,p) \rVert}$.

((\ref{lem:dirreconstr} is also homeomorphism))
...
\begin{proof}
    ...
    \]i.e. $q_\times\coloneqq \gamma_{p^i_0,v^i_0}(-t^i_\times)$ for $i=1,2$. But this is a contradiction because $p^1_0$ and $p^2_0$ are in $\er(q_0)$ and thus cannot be cut points of $q_0$.
\end{proof}
